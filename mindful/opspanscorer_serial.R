...
for(qn in recall.qns){
  ...
  for(response in responses){
    words <- strsplit(response,', ',fixed=TRUE); words <- unique(words[[1]])
    current_score <- 0
    for(i in 1:length(answers)){
      if(i <= length(words)){
        if(words[i] == answers[i]){current_score <- current_score + 1}
      }
    }; rm(i)
...
write.csv(export, paste0("scored_serial_", data_file_name), row.names = F)
